...
A modern, responsive portfolio built with Vite, React, TypeScript, and Tailwind CSS. It showcases skills, experience, and selected projects in DevOps, Cloud, Containers, and Automation. The site supports dark mode, smooth interactions, and includes a minimal backend endpoint (FastAPI) for contact form submissions to a Supabase table.
...
kimani-portfolio/
├─ api/
│  └─ submit-message.py        # FastAPI app to accept contact messages to Supabase
├─ src/
...
├─ requirements.txt            # Python deps for FastAPI backend
...
- hCaptcha is optional; if unset, captcha validation is skipped.
...
- Inserts the message into `messages` table via Supabase REST API
